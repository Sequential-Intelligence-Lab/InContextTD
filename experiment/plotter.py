...
def plot_error_data(xs: np.ndarray,
    ...
    std_vf_sim_smooth = smooth_data(std_vf_sim, 5)
    mean_zo_cos_sim_smooth = smooth_data(mean_zo_cos_sim, 5)
    std_zo_cos_sim_smooth = smooth_data(std_zo_cos_sim, 5)
    mean_fo_cos_sim_smooth = smooth_data(mean_fo_cos_sim, 5)
    std_fo_cos_sim_smooth = smooth_data(std_fo_cos_sim, 5)
    mean_sensitivity_cos_sim_smooth = smooth_data(mean_sensitivity_cos_sim, 5)
    std_sensitivity_cos_sim_smooth = smooth_data(std_sensitivity_cos_sim, 5)
    ...
    ax1.plot(xs, mean_zo_cos_sim_smooth, label='0th Order', color=sns.color_palette()[0])
    ax1.fill_between(xs, mean_zo_cos_sim_smooth - std_zo_cos_sim_smooth,
                     mean_zo_cos_sim_smooth + std_zo_cos_sim_smooth, lw=0, alpha=0.2, color=sns.color_palette()[0])
    a, = ax1.plot(xs, mean_sensitivity_cos_sim_smooth, label='Sensitivity', color=sns.color_palette()[3])
    ax1.fill_between(xs, mean_sensitivity_cos_sim_smooth - std_sensitivity_cos_sim_smooth,
                     mean_sensitivity_cos_sim_smooth + std_sensitivity_cos_sim_smooth, lw=0, alpha=0.2, color=sns.color_palette()[3])

    ax2 = ax1.twinx()
    ax2.set_ylabel('MSVE')
    b, = ax2.plot(xs, mean_vf_sim_smooth, label='MSVE', color=sns.color_palette()[2])
    ax2.fill_between(xs, mean_vf_sim_smooth - std_vf_sim_smooth,
                     mean_vf_sim_smooth + std_vf_sim_smooth, lw=0, alpha=0.2, color=sns.color_palette()[2])
...
if __name__ == '__main__':
    runs_directory = os.path.join(
        './logs', 'linear_discounted_train', '2024-05-08-18-43-22')
