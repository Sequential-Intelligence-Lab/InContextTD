...
class LinearTransformer(nn.Module):
    def __init__(self,
        ...
        if mode == 'auto':
            ...
            self.layers = nn.ModuleList([LinearAttention(d, n, lmbd) for _ in range(l)])
            for i, attn in enumerate(self.layers):
                nn.init.xavier_normal_(attn.P, gain=0.1/(i+1))
                nn.init.xavier_normal_(attn.Q, gain=0.1/(i+1))
